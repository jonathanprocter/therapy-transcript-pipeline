...
def process_new_files():
    """Process any new files found in Dropbox"""
    try:
        from services.dropbox_service import DropboxService
        from services.ai_service import AIService
        from services.document_processor import DocumentProcessor
        from models import Transcript, Client, ProcessingLog, db
        from datetime import datetime, timezone
        import tempfile
        import os
        ...
        if not dropbox_service.test_connection():
        ...
        for file_info in new_files[:5]:  # Process 5 files at a time
            try:
                logger.info(f"Processing file: {file_info['name']}")
                ...
                try:
                    ...
                    text_content = doc_processor.extract_text_from_file(temp_path)
                    if not text_content:
                        continue

                    # Extract client name from filename or content
                    client_name = extract_client_name_from_filename(file_info['name'])
                    if not client_name:
                    ...
                    log_entry = ProcessingLog(
                        ...
                        status='info',
                        message=f'Started processing {file_info["name"]}',
                        created_at=datetime.now(timezone.utc)
                    )
                    ...
                    if ai_service and ai_service.openai_client:
                        try:
                            analysis = ai_service.analyze_transcript(text_content)
                            if analysis:
                                transcript.openai_analysis = analysis.get('analysis', '')
                                transcript.sentiment_score = analysis.get('sentiment_score', 0.0)
                                transcript.key_themes = analysis.get('key_themes', '')
                                transcript.therapy_insights = analysis.get('therapy_insights', '')
                                transcript.progress_indicators = analysis.get('progress_indicators', '')
                                transcript.processing_status = 'completed'
                                transcript.processed_at = datetime.now(timezone.utc)
...
def extract_client_name_from_filename(filename):
    ...
    try:
        ...
        if len(parts) >= 2:
            # Look for name pattern before date/time
            potential_name = f"{parts[0]} {parts[1]}"

            # Check if next part looks like date/time
            if len(parts) > 2:
                third_part = parts[2]
                if any(char in third_part for char in ['/', '-', ':', '2025', '2024', '2023']):
